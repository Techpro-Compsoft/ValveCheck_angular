...
@Component({
  ...
  templateUrl: './supervisor-report.page.html',
  styleUrls: ['./supervisor-report.page.scss'],
})
export class SupervisorReportPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.supervisorName = data.fullname;
  ...
  writeFileNow(path, csv) {
    this.file.writeFile(`${path}/valveCheck reports`, `${this.supervisorName}_${this.selectedDate}.csv`, csv, { replace: true }).then(res => {
      if (res) {
